...
void FkRenderTexQuark::describeProtocols(std::shared_ptr<FkPortDesc> desc) {
    ...
    FK_PORT_DESC_QUICK_ADD(desc, FkNewImageSourceTexProto, FkRenderTexQuark::_onAllocTexWithSource);
...
FkResult FkRenderTexQuark::_onAllocTex(const std::shared_ptr<FkProtocol> &p) {
    ...
    auto imageSourceCompo = FK_FIND_COMPO(proto->texEntity, FkImageSourceCompo);
    if (bufCompo) {
        _updateTexWithBuf(texArray, proto->texEntity, bufCompo->buf);
    } else if (imageSourceCompo) {
        if (!texArray->textures.empty()) {
            imageSourceCompo->value->onCreate(texArray->textures[0]->tex, 0, "");
            texArray->imageSource = imageSourceCompo->value;
        }
...
FkResult FkRenderTexQuark::_onAllocTexWithSource(const std::shared_ptr<FkProtocol> &p) {
    return _onAllocTex(p);
}
...
std::shared_ptr<FkTexArrayCompo> FkRenderTexQuark::_allocTex(std::shared_ptr<FkTexEntity> &texEntity) {
    ...
    auto imageSourceCompo = FK_FIND_COMPO(texEntity, FkImageSourceCompo);
    FkTexDescription desc(GL_TEXTURE_2D);
    auto blocks = _calcTexBlock(texEntity->size(), blockSize);
    if (imageSourceCompo) {
        blocks = {1, 1};
        if (imageSourceCompo && imageSourceCompo->value
            && imageSourceCompo->value->getTexDescription().target == GL_TEXTURE_EXTERNAL_OES) {
    ...
    for (int y = 0; y < texArray->blocks.y; ++y) {
        ...
        for (int x = 0; x < texArray->blocks.x; ++x) {
            ...
            if (fboCompo && buf) {
                ...
                FkLogI("filmkilns", "%d, %d", index, ret);
...
FkResult FkRenderTexQuark::_onRemoveTex(std::shared_ptr<FkProtocol> &p) {
    ...
    if (itr != sMap.end()) {
        auto texArrayCompo = itr->second;
        if (texArrayCompo && texArrayCompo->imageSource) {
            texArrayCompo->imageSource->onDestroy();
        }
        sMap.erase(itr);
    }
    return FK_OK;
