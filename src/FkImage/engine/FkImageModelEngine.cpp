...
FkResult FkImageModelEngine::_writeModel2File(std::string &dir,
    ...
    if (model->layers().empty()
        || model->canvas().size().width() <= 0 || model->canvas().size().height() <= 0) {
        FkLogI(FK_DEF_TAG, "Write model fail cause by empty model.");
        return FK_EMPTY_DATA;
    }
    _printModel("Write model", model);
    auto modelFile = dir;
...
void FkImageModelEngine::_printModel(const std::string &tag, std::shared_ptr<pb::FkPictureModel> &model) {
    FkLogI(FK_DEF_TAG, "%s: Canvas(id=%d, size=%dx%d)", tag.c_str(),
           model->canvas().id(), model->canvas().size().width(), model->canvas().size().height());
    for (auto &layer: model->layers()) {
        FkLogI(FK_DEF_TAG, "%s: Layer(id=%d, size=%dx%d, file=%s)", tag.c_str(),
               layer.id(), layer.size().width(), layer.size().height(), layer.file().c_str());
    }
}
