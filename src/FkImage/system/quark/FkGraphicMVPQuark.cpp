...
FkResult FkGraphicMVPQuark::_onRenderRequest(std::shared_ptr<FkProtocol> p) {
    ...
    if (isRenderOnce) {
        double scaleX = proto->req->strategy.outSize.getWidth() * 1.0 / sizeComp->size.getWidth();
        double scaleY = proto->req->strategy.outSize.getHeight() * 1.0 / sizeComp->size.getHeight();
        scale = std::max(scaleX, scaleY);
        canvas->addComponent(std::make_shared<FkSizeCompo>(proto->req->strategy.outSize));
    }
    sizeComp = FK_FIND_COMPO(canvas, FkSizeCompo);
    ...
    for (auto &layer : proto->req->layers) {
        ...
        if (isRenderOnce) {
            ...
            if (scaleCompo) {
                auto newScaleCompo = std::make_shared<FkScaleComponent>(*scaleCompo);
                newScaleCompo->value.x *= scale;
                newScaleCompo->value.y *= scale;
                layer->addComponent(newScaleCompo);
            }
        }
...
FkResult FkGraphicMVPQuark::_calc(std::shared_ptr<FkGraphicLayer> layer,
    ...
    auto imageSourceCompo = FK_FIND_COMPO(layer, FkImageSourceCompo);
    if (imageSourceCompo) {
        auto transMat = imageSourceCompo->value->getTransformMatrix();
        if (transMat && FK_INSTANCE_OF(transMat, FkGLMatrix)) {
            transMatCompo = std::make_shared<FkTransMatCompo>();
            transMatCompo->value = transMat;
        }
    }
    if (transMatCompo == nullptr) {
        transMatCompo = std::make_shared<FkTransMatCompo>();
        transMatCompo->value = std::make_shared<FkGLMatrix>();
    }
    layer->addComponent(transMatCompo);
    return FK_OK;
}
